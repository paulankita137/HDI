...
#names = [10846,	10907,	11026,	11074,	11162,	11392,	11425,	11445,	11615,	11617,	11766,	11772,	11822,	11840,	12135,	12232,	12325,	12529,	12587,	12701,	12874,	13052,	13061,	13070,	13159,	13334,	13385,	13652,	13705,	13957,	14552,	14645,	14652,	14667,	14785,	14849,	15004,	15063,	15155,	15200,
#]

#names = [11026]

names= [10119,  10191,  10198,  10226,  10241,  10265,  10266,  10313,  10423,10771,  10783,  10846,  10907,  11026,  11074,  11162,  11392,  11425,  11445,  11615,  11617,  11766,  11772,  11822,  11840,  12135,  12232,  12325,  12529,  12587,  12701,  12874,  13052,  13061,  13070,  13159,  13334,  13385,  13652,  13705,  13957,  14552,  14645,  14652,  14667,  14785,  14849,  15004,  15063,  15155,  15200,  15320,  15398,  15449,  15456,  15468,  15491,  15800,  15843,  15900,  16199,  16210,  16244,  16531,  16546,  16646,  16825,  16927,  17080,  17092,  17286,  17297,  17331,  17343,  17368,  17411,  17551,  17721,  17930,  17945,  18021,  18102,  18190,  18405,  18505,  18554,  18687,  18888,  19046,  19054,  19067,  19218,  19226,  19537,  19707,  19793,  19797,  19821,  19836,  20238,  20312,  20319,  20416,  20624,  20633,  20689,  20771,  21076,  21135,  21200,  21264,  21400,  21585,  21784,  21905,  21949,  21958,  22174,  22234,  22258,  22292,  22331,  22389,  22546,  22726,  22834,  22867,  22875,  22945,  23152,  23154,  23504,  23531,  23580,  23581,  23700,  23719,  23794,  23814,  23828,  23906,  23913,  24007,  24073,  24080,  24253,  24285,  24370,  24396,  24397,  24541,  24560,  24648,  24685,  24698,  24927,  25122,  25159,  25176,  25305,  25314,  25804,  25996,  26214,  26252,  26411,  26433,  26506,  26603,  26739,  26863,  27030,  27192,  27221,  27408,  27484,  27536,  27543,  27576,  27863,  27909,  27924,  27964,  28023,  28131,  28181,  28195,  28205,  28237,  28239,  28248,  28271,  28286,  28392,  28583,  28641,  28660,  28695,  28817,  28891,  28961,  29285,  29310,  29460,  29487,  29490,  29526,  29534,  29593,  29689,  29698,  29731,  29869,  29931,  29990,  29992,  30051,  30174,  30605,  30935,  30954,  31025,  31026,  31106,  31178,  31241,  31340,  31359,  31447,  31546,  31717,  31738,  31883,  31901,  31911,  31994,  32039,  32080,  32143,  32198,  32239,  32292,  32326,  32345,  32525,  32537,  32575,  32620,  32715,  32779,  32847,  32857,  32991,  33144,  33173,  33181,  33229,  33427,  33492,  33499,  33671,  33693,  33740,  33817,  33842,  33909,  34228,  34263,  34405,  34419,  34490,  34553,  34668,  34683,  34762,  34783,  34841,  34964,  35023,  35222,  35355,  35371,  35462,  35473,  35792,  35941,  36097,  36186,  36568,  36572,  36785,  36831,  36849,  36850,  36998,  37042,  37043,  37068,  37090,  37137,  37306,  37317,  37578,  37749,  38114,  38190,  38424,  38576,  38602,  38628,  38751,  38757,  38793,  38807]
...
for i in range (0,len(names)):
    ...
    df=pd.read_csv('wsc-visit1-'+str(names[i])+'-nsrr.edfspo2_'+'100sps.csv',sep=';')
    #df=pd.read_csv('*.edfspo2*.csv',sep=';')
    X=df.iloc[:,1]
    ...
    desaturation_area,desaturation_area_lg,hypoxic_burden,hypoxic_burden_lg,eightyeight,sleeptime,timelists=HBFunction.hbfunction(dataset)
    Desat_Area.append(desaturation_area)
    np.savetxt("desatarea"+str(names[i])+".csv", Desat_Area, delimiter=",")


    Desat_Area_Log.append(desaturation_area_lg)
    np.savetxt("desat_log"+str(names[i])+".csv", Desat_Area_Log, delimiter=",")
    Hyp_Burden.append(hypoxic_burden_lg)
    np.savetxt("Hyp"+str(names[i])+".csv", Hyp_Burden, delimiter=",")
    #Baseline.append(baseline)
    #np.savetxt("Baseline.csv", Baseline, delimiter=",")
    EightyEight.append(eightyeight)
    np.savetxt("Eightyeight"+str(names[i])+".csv", EightyEight, delimiter=",")
    SleepTime.append(sleeptime)
    np.savetxt("SleepTime"+str(names[i])+".csv", SleepTime, delimiter=",")
    Timelist.append(timelists)
    #print('Timelist',Timelist)
    #np.savetxt("timelist.csv", Timelist, delimiter=",")
    Tl = pd.DataFrame(Timelist)
    print('Timelist',Tl)
    Tl.to_csv('Timelist'+str(names[i])+'.csv', sep=';',encoding='utf-8', index=None,header=True)
    percentile_list = pd.DataFrame(
                {'Desaturation Area': Desat_Area,
                 'DA Log':Desat_Area_Log,  
                 'Time Spent below 88': EightyEight,
                 'Total Sleep Time':SleepTime
                          
                             })
    result = pd.concat([percentile_list, Tl], axis=1, join='inner')

    result.to_csv('Full-List'+str(names[i])+'.csv', sep=';',encoding='utf-8', index=None,header=True)
